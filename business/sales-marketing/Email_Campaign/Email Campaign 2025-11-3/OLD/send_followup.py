...
def get_emails_sent_today(message_type='followup') -> int:
    """
...
def send_batch(prospects_by_vertical: Dict[str, pd.DataFrame], followup_candidates: Dict[str, List[Dict]], daily_limit_remaining: int) -> int:
    ...
    while total_sent < total_to_send and not interrupted:
        ...
        if success:
            ...
            # Count from tracker to ensure accuracy even across restarts
            total_sent_today = get_emails_sent_today('followup')
            coordination.update_sent_count('followup', total_sent_today)
            ...
            print(f"[{timestamp}] ✓ Follow-up sent to {email} ({vertical_name}, {first_name_display}) ({total_sent}/{total_to_send} this run)")
...
def main(dry_run: bool = False):
    ...
    # Display candidates by vertical (only active verticals in production mode)
    print("\n📊 Follow-up candidates by vertical:")
    if config.TEST_MODE:
        display_verticals = config.VERTICALS.keys()
    else:
        display_verticals = config.ACTIVE_VERTICALS
    for vertical in display_verticals:
    ...
    # Get user approval before proceeding (CRITICAL: prevents accidental sends)
    print("\n╔══════════════════════════════════════════════════════════════════╗")
    print("║  FOLLOW-UP SEND APPROVAL                                         ║")
    print("╠══════════════════════════════════════════════════════════════════╣")

    # Only count/show candidates from active verticals
    active_candidates = {v: followup_candidates.get(v, []) for v in display_verticals}
    total_followups = sum(len(candidates) for candidates in active_candidates.values())
    print(f"║  Total follow-ups to send: {total_followups:<43} ║")
    print("║                                                                  ║")
    print("║  Sample recipients (first 10):                                   ║")
    sample_count = 0
    for vertical in display_verticals:
        if vertical not in active_candidates:
            continue
        for candidate in active_candidates[vertical][:10 - sample_count]:
            email = candidate['email']
            print(f"║    {email[:62]:<62}║")
            sample_count += 1
            if sample_count >= 10:
    ...
    if user_input in ['yes', 'approve', 'y']:
        print("✅ Approved! Starting follow-up campaign...\n")
    else:
    ...
    while not interrupted:
        ...
        # Calculate end of business hours for today
        now = get_current_time_est()
        end_time = now.replace(hour=config.BUSINESS_HOURS_END, minute=0, second=0, microsecond=0)
        ...
        print("\n╔════════════════════════════════════════════════════════════╗")
        print("║  TODAY'S ALLOCATION (FOLLOW-UP from coordination)         ║")
        print("╠════════════════════════════════════════════════════════════╣")
    ...
    shutdown_hooks.run_shutdown_hooks('followup', check_bounces=True, sync_crm=True)
    print(f"\n✅ Sent {emails_sent_this_session} follow-ups this session.")
    print("💡 Run script again to pick up where you left off.\n")

if __name__ == "__main__":
    # Parse command line arguments
    parser = argparse.ArgumentParser(
        description='Email Validation Campaign System - Follow-up',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog='''
Examples:
  python send_followup.py              # Normal run with user approval
  python send_followup.py --dry-run    # Preview mode, show stats but don't send
