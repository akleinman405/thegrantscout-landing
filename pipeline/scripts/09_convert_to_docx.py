...
def add_styled_table(doc, rows):
    ...
    - Tables don't split across pages (keep rows together)
    """
    if not rows:
        return
    ...
    for i, row_data in enumerate(rows):
        ...
        # Set "keep with next" on all rows except the last
...
def get_indent_level(line):
    """Count leading spaces to determine indent level (each 2-3 spaces = 1 level)."""
    stripped = line.lstrip()
    leading_spaces = len(line) - len(stripped)
    return leading_spaces // 2  # 2 spaces per indent level
...
def convert_markdown_to_docx(md_path, docx_path):
    ...
    while i < len(lines):
        ...
        if line.startswith('# ') and not line.startswith('## '):
            ...
            # NO page break here - title and TOC stay on page 1
            # Page break will be added before "If You Only Do One Thing" section
            i += 1
            break
        elif line:
    ...
    while i < len(lines):
        ...
        if line.startswith('## '):
            ...
            # Add page break before "If You Only Do One Thing" (first content section after TOC)
        ...
        # Bullet points (including nested/indented)
        # Check for bullets with leading spaces (nested) or at start
        original_line = lines[i]  # Get original line with spaces preserved
        bullet_match = re.match(r'^(\s*)-\s+(.*)$', original_line)
        if bullet_match:
            indent_spaces = len(bullet_match.group(1))
            bullet_text = bullet_match.group(2)
            # Clean markdown links
            bullet_text = clean_markdown_link(bullet_text)

            # Use appropriate bullet style based on indent level
            # indent_level 0 = List Bullet, 1+ = List Bullet 2 (with additional indent)
            indent_level = indent_spaces // 2
            if indent_level == 0:
                para = doc.add_paragraph(style='List Bullet')
            else:
                # Use List Bullet 2 for nested items (different bullet character)
                try:
                    para = doc.add_paragraph(style='List Bullet 2')
                except KeyError:
                    # Fallback if List Bullet 2 doesn't exist
                    para = doc.add_paragraph(style='List Bullet')
        ...
        if re.match(r'^\d+\.\s', line):
            ...
            # Clean markdown links
            list_text = clean_markdown_link(list_text)
            para = doc.add_paragraph(style='List Number')

            # Handle inline bold within list text
