# R_2025-01-02_lasso_v5.R
# Train LASSO V5 model on first-time grants with new features
#
# Key changes from V4:
# - Training on first-time grants only (not all grants)
# - Accessible foundations only (filtered pre-training)
# - Grant type features (pct_capital, pct_operating, pct_program)
# - Interaction terms (match_state_x_geo_conc, match_sector_x_sector_hhi)
# - Expected: openness_score coefficient should be POSITIVE
#
# Usage:
#   Rscript scripts/r/R_2025-01-02_lasso_v5.R
#
# Prerequisites:
#   - Run SQL_2025-01-02_v5_training_data.sql
#   - Run PY_2025-01-02_v5_export_training.py
...
INPUT_DIR <- file.path(BASE_DIR, "outputs/v5")
OUTPUT_DIR <- file.path(BASE_DIR, "outputs/v5/model")

# Create output directory
dir.create(OUTPUT_DIR, recursive = TRUE, showWarnings = FALSE)

cat("=" , rep("=", 59), "\n", sep = "")
cat("LASSO V5 Model Training\n")
cat("Training on first-time grants from accessible foundations\n")
cat("=" , rep("=", 59), "\n\n", sep = "")
...
plot(cv_fit, main = "LASSO V5 Cross-Validation")
dev.off()
...
# ============================================================================
# KEY CHECK: Openness Score Coefficient
# ============================================================================

cat("\n", rep("=", 60), "\n", sep = "")
cat("KEY COEFFICIENTS (Sanity Check)\n")
cat(rep("=", 60), "\n", sep = "")

key_features <- c("f_openness_score", "f_repeat_rate", "match_sector_pct",
                  "match_state_pct", "match_same_state", "f_pct_capital",
                  "f_log_total_grants", "f_sector_hhi")

for (feat in key_features) {
  coef_val <- coef_df$coefficient[coef_df$feature == feat]
  if (length(coef_val) > 0 && coef_val != 0) {
    sign_str <- if (coef_val > 0) "POSITIVE" else "NEGATIVE"
    cat(sprintf("  %-25s: %+.4f (%s)\n", feat, coef_val, sign_str))
  } else {
    cat(sprintf("  %-25s: 0 (zero/not found)\n", feat))
  }
}

# Critical check
openness_coef <- coef_df$coefficient[coef_df$feature == "f_openness_score"]
if (length(openness_coef) > 0 && openness_coef > 0) {
  cat("\n*** SUCCESS: f_openness_score is POSITIVE! ***\n")
} else if (length(openness_coef) > 0 && openness_coef < 0) {
  cat("\n*** WARNING: f_openness_score is still NEGATIVE ***\n")
} else {
  cat("\n*** WARNING: f_openness_score was zeroed out by LASSO ***\n")
}
...
precision <- sum(pred_class == 1 & y_val == 1) / sum(pred_class == 1)
recall <- sum(pred_class == 1 & y_val == 1) / sum(y_val == 1)
...
plot(roc_test, main = sprintf("LASSO V5 ROC Curve (Test AUC = %.3f)", auc(roc_test)),
     col = "blue", lwd = 2)
...
hist(pred_test, breaks = 50, main = "V5 Score Distribution (Test Set)",
...
cat(sprintf("  Model:              LASSO V5 (first-time grants)\n"))
...
cat("\nNext steps:\n")
cat("  1. Review coefficients - verify f_openness_score is positive\n")
cat("  2. Copy coefficients.json and scaling.json to Pipeline v2/config/\n")
cat("  3. Update 02_score_foundations.py to use new features\n")
cat("  4. Test on Ka Ulukoa and PSMF\n")
