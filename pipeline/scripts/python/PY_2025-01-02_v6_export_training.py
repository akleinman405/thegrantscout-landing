#!/usr/bin/env python3
"""
PY_2025-01-02_v6_export_training.py
Export V6 training data with semantic similarity and cleaned features.

Key changes from V5:
- REMOVED: r_total_grants, r_total_funders, r_total_funding, r_assets, r_employee_count
- ADDED: Semantic similarity (foundation avg embedding vs average recipient in sector)
- ADDED: Ask amount alignment features
- ADDED: Organization age features
- CLEANED: No negative revenue rows

Usage:
    python3 scripts/python/PY_2025-01-02_v6_export_training.py
...
def load_sector_avg_embeddings():
    ...
    # Get average embedding per sector from grants
    query = """
    SELECT
        LEFT(fg.recipient_ntee_code, 1) as ntee_broad,
        AVG(ge.purpose_embedding[1:10]) as sample_embedding  -- Just first 10 dims for speed
    FROM f990_2025.fact_grants fg
    JOIN f990_2025.emb_grant_purposes ge ON fg.id = ge.grant_id
    WHERE fg.recipient_ntee_code IS NOT NULL
      AND ge.purpose_embedding IS NOT NULL
    GROUP BY LEFT(fg.recipient_ntee_code, 1)
    """

    # This query would be too slow, so we'll use foundation embeddings grouped by sector instead
    query = """
    ...
    SELECT
        fs.ntee_broad,
        AVG(fae.avg_embedding) as avg_embedding
    ...
    try:
        ...
        embeddings = {}
        for _, row in df.iterrows():
            if row['avg_embedding'] is not None:
                embeddings[row['ntee_broad']] = np.array(row['avg_embedding'])
        log(f"Loaded {len(embeddings)} sector average embeddings")
...
def add_derived_features(df, foundation_embeddings, sector_embeddings):
    ...
    # =========================================================================
    # Ratio of recipient budget to foundation's typical grant
    # Ideal: recipient budget is 10-100x the grant size
...
def impute_missing(df):
    ...
    df['f_funded_new_recently'] = df['f_funded_new_recently'].fillna(0)
    df['f_is_growing'] = df['f_is_growing'].fillna(0)
    df['f_is_declining'] = df['f_is_declining'].fillna(0)
    df['f_first_time_rate'] = df['f_first_time_rate'].fillna(0.3)
    df['f_one_time_rate'] = df['f_one_time_rate'].fillna(0.5)
...
def main():
    ...
    df_features.to_csv(OUTPUT_DIR / 'training_data_v6.csv', index=False)
    ...
    log("Done!")
    print(f"\nOutput files:")
    print(f"  {OUTPUT_DIR / 'training_data_v6.csv'}")
    ...
    print("\n" + "=" * 60)
    print("V6 vs V5 Feature Comparison:")
    print("=" * 60)
    print("\nREMOVED from V5:")
    print("  - r_total_grants (leakage)")
    ...
    print("  - f_is_small")
    print("  - f_funded_new_recently (funded new recipient 2023-2024)")
    print("  - f_is_growing, f_is_declining (giving trend)")
    print("  - f_first_time_rate (historical new recipient rate)")
    print("  - f_one_time_rate (one-time grant rate)")
    print("  - f_is_recently_active (gave grants 2023-2024)")
    print("  - match_same_region")
    print("  - match_same_division")
